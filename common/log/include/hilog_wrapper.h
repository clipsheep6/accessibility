...
#define _FILENAME_ (__builtin_strrchr(__FILE__, '/') ? __builtin_strrchr(__FILE__, '/') + 1 : __FILE__)

#define HILOG_FATAL(fmt, ...)            \
    ((void)HILOG_IMPL(LOG_CORE, LOG_FATAL, AAMS_LOG_DOMAIN, AAMS_LOG_TAG, \
    "[%{public}s(%{public}s:%{public}d)]" fmt, _FILENAME_, __FUNCTION__, __LINE__, ##__VA_ARGS__))

...
Rect AccessibilityWindowManager::RosenToAccessibilityRect(const Rosen::Rect& rect) {
    Rect rectTemp;
    ...
    const sptr<Rosen::AccessibilityWindowInfo> windowInfo)
{
    ...
    if (static_cast<int32_t>(windowInfo->type_) == 1 && (static_cast<int32_t>(windowInfo->windowRect_.width_) == 0 ||
        ...
        Rect reRect = RosenToAccessibilityRect(windowInfo->windowRect_);
        accWindowInfo.SetRectInScreen(reRect);
    }
    ...
    for (auto &rect : windowInfo->touchHotAreas_) {
        ...
        Rect reRect = RosenToAccessibilityRect(windowInfo->windowRect_);
        tempTouchHotAreas.push_back(reRect);
    }

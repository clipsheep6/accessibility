    ...
    const int32_t windowId, const sptr<AccessibilityWindowConnection>& interactionConnection)
{
    ...
    HILOG_INFO("after add connection interactionOperators's size[%{public}d]", asacConnections_.size());
...
void AccessibilityAccountData::RemoveAccessibilityWindowConnection(const int32_t windowId)
{
    ...
    HILOG_INFO("after remove connection interactionOperators's size[%{public}d]", asacConnections_.size());
    for (auto &asacConnection : asacConnections_) {
        HILOG_INFO("The window id of asacConnection is %{public}d", asacConnection.first);
    }
}
...
{
    HILOG_INFO("windowId[%{public}d] interactionOperators's size[%{public}d]", windowId, asacConnections_.size());
    for (auto &asacConnection : asacConnections_) {
        HILOG_INFO("The window id of asacConnection is %{public}d", asacConnection.first);
    }
